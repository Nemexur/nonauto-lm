...
class FeedForward(torch.nn.Module, FromParams):
    ...
    --------
    ```python
    FeedForward(124, 2, [64, 32], Activation.by_name('relu'), 0.2)
    #> FeedForward(
    ...
    def __init__(
        ...
        if len(hidden_sizes) != num_layers:
            raise ValueError(
                f"len(hidden_dims) ({len(hidden_sizes)}) != num_layers ({num_layers})"
            )
        if len(activations) != num_layers:
            raise ValueError(
                f"len(activations) ({len(activations)}) != num_layers ({num_layers})"
            )
        if len(dropout) != num_layers:
            raise ValueError(
                f"len(dropout) ({len(dropout)}) != num_layers ({num_layers})"
            )
        self._input_size = input_size
        self._output_size = output_size or hidden_sizes[-1]
        # Modules
        ...
        self._output = torch.nn.Linear(hidden_sizes[-1], self._output_size)
